...
class ShopcartController extends AbstractController
{
    ...
    public function index(ShopcartRepository $shopcartRepository): Response
    {
        ...
        $sql = "SELECT p.title, p.sprice, s.* FROM shopcart s, product p WHERE s.productid = p.id";
        $statement = $em->getConnection()->prepare($sql);
        $statement->bindValue('userid', $user->getid());
        $statement->execute();
        $shopcart = $statement->fetchAll();

        return $this->render('shopcart/index.html.twig', ['shopcarts' => $shopcart, 'cats' => $cats,
]);
    }
